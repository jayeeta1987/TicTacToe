...
trackGridPressed = {'grid1': False, 'grid2': False, 'grid3': False, 'grid4': False, 'grid5': False, 'grid6': False,
                    'grid7': False, 'grid8': False, 'grid9': False}
...
# showing player 2 name in the window left hand corner at bottom
...
def resetGridButtonState():
    ...
    for key in gridButtons:  # for loop on dictionar gridButton.
...
def createMenuItems(window, resetNewGame):
    ...
    filemenu.add_command(label="New Game", command=lambda: StartFromBegin(
        window))  # add label 'New Game' to filemenu and bind with method 'StartFromBegin'
    filemenu.add_command(label="Reset Game", command=lambda: newGame(window,
                                                                     resetNewGame))  # add label 'Reset Game' to filemenu and bind with method 'newGame'
    filemenu.add_separator()  # Add separator for filemenu items
    filemenu.add_command(label="Exit",
                         command=lambda: Exit(window))  # add label 'Exit' to filemenu and bind with method 'Exit'
...
def About():
    messagebox.showinfo("About Game",
                        "Tic Tac Toe is to get three in a row. You play on a three by three game board. The first player is known as X and the second is O. Players alternate placing Xs and Os on the game board until either opponent has three in a row or all nine squares are filled.")
