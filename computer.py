...
import random
import drawframe #for drawframe
    ...
    #If grid button not pressed before then it will allow to update values 
        if(not trackGridPressed[gridKey]):
            ...
            if(userWinConfig()):
                ...
                updateScore(player.get('player1')['id'], player.get('player1')['score']) #update score acccording to id
...
def computerTurn(window,score):
    ...
    if(userWinConfig()): #if computer wins it will update the messagebox with wiining message. label,score will be updated also.
        ...
        updateScore(player.get('player2')['id'], player.get('player2')['score']) #Update the score according to id
...
def Exit(window):
    print("Exit")
    window.destroy()
#function 'TopTenScore' for score update    
def TopTenScore():
    ...
    messagebox.showinfo("Top Scores",msg)  #score display in messagebox.           
